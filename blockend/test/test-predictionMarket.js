...
async function main() {
    ...
    const factory = await factory_contract.deploy(
        ethers.utils.parseEther("0.01")  // 0.01 ETH minimum deposit
    );
    ...
    for (let i = 1; i <= 20; i++) {
        ...
        bettingTxPromises.push(
            market1.connect(accounts[i]).placeBet(outcome, { 
    ...
    console.log('\n📊 Current Odds:');
    console.log(`  Outcome 0: ${(odds[0]/100).toFixed(2)}%`);
    console.log(`  Outcome 1: ${(odds[1]/100).toFixed(2)}%`);
    console.log(`  Outcome 2: ${(odds[2]/100).toFixed(2)}%`);
    ...
    tx = await manager.connect(accounts[25]).batchBet(marketIds, outcomes, amounts, {
        value: totalAmount,
        gasLimit: 5000000
    });
    await tx.wait();
    ...
    const portfolioMarketCount = await manager.callStatic.getPortfolio(marketIds, accounts[25].address);
    console.log(`\n📊 Portfolio for user ${accounts[25].address}:`);
    ...
    tx = await market1.connect(accounts[0]).resolveMarket(0);
    await tx.wait();
    console.log('✅ Market 1 resolved: Outcome 0 wins');
    
    tx = await market2.connect(accounts[0]).resolveMarket(0);
    await tx.wait();
    console.log('✅ Market 2 resolved: Outcome 0 wins');
    
    tx = await market3.connect(accounts[0]).resolveMarket(1);
    await tx.wait();
